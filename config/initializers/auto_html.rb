AutoHtml.add_filter(:responsiveflickr).with(:maxwidth => nil, :maxheight => nil, :link_options => {}) do |text, options|
  ...
  text.gsub(regex) do |match|
    ...
    %{<a href="#{match}"#{ ' ' + link_options unless link_options.empty? }><img src="#{response["url"]}" alt="#{response["title"]}" title="#{response["title"]}" class='img-responsive' /></a>}
  end
end